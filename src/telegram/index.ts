...
export class TelegramManager {
  ...
  private setupCommands() {
    ...
    this.bot.onText(/\/trade(?:\s+(\w+))?/, async (msg, match) => {
      if (!this.isAuthorizedChat(msg.chat.id)) return;
      const type = match?.[1];

      if (type !== "virtual" && type !== "real") {
        const message = "tipo de trade inválido!";
        this.bot.sendMessage(msg.chat.id, message, { parse_mode: "Markdown" });
        return;
      }

      const { loss, totalTrades, win, winRate } =
        type === "virtual"
          ? this.tradeWinRateService.getVirtualStats()
          : this.tradeWinRateService.getTradeStats();

      const message = `*📊 Estatísticas de Trading ${type}*\n\n`+
      `*Total de Trades:* ${totalTrades}\n`+
      `*Vitórias:* ${win}\n`+
      `*Derrotas:* ${loss}\n`+
